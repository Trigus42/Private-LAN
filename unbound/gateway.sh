...
# Move rules from gateway-firewall.sh to the top again
iptables -D FORWARD -i "$lan_if" -o "$lan_if" -j REJECT
iptables -D FORWARD -s 172.16.238.0/24 ! -d "$lan_subnet" -o "$lan_if" -j REJECT
iptables -D FORWARD -s 172.16.238.2 -o "$lan_if" -j ACCEPT
iptables -D FORWARD -s 172.16.238.8 -o "$lan_if" -j ACCEPT
...
if ! eval "iptables -C FORWARD -i "$lan_if" -o "$lan_if" -j REJECT &> /dev/null"; then
    iptables -I FORWARD -i "$lan_if" -o "$lan_if" -j REJECT
fi
